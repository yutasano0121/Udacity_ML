...
def model_fn(model_dir):
    ...
    return model.to(device)  # training mode


def _get_train_loader(batch_size, data_dir):
    print("Get a data loader.")
    ...
    x_train = torch.from_numpy(train_data.drop([0], axis=1).values).float()
...
def save_model_params(model, model_dir):
    # 'args' is defined within the __main__ block.
...
def train(model, train_loader, epochs, optimizer, criterion, device):
    ...
    for epoch in range(1, epochs + 1):
        model.train()
        total_loss = 0,
        # Make an estimate by batch and sum up losses.
    ...
    # Save the model after all epochs are run.
